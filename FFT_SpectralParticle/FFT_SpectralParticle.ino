// Spectral particle FFT hardware //
...
void loop() {
    ...
    for (int i = 0; i < FFT_N / 2; i++) {
        ...
        float decay = (i > BINS/4 && i < BINS/2) ? 0.4f : 0.98f;
        
        fft_in_data[i].R1 = (int16_t)(fft_in_data[i].R1 * decay);
        fft_in_data[i].I1 = (int16_t)(fft_in_data[i].I1 * decay);
        fft_in_data[i].R2 = (int16_t)(fft_in_data[i].R2 * decay);
        fft_in_data[i].I2 = (int16_t)(fft_in_data[i].I2 * decay);

    }

    if (rand() % 10 > 7) {

        int target_bin = (rand() % (BINS/4)) + (BINS/4); 
        int16_t amp = rand() % (AMPS * 2);
        if (target_bin % 2 == 0) fft_in_data[target_bin / 2].R1 += amp;
        else fft_in_data[target_bin / 2].R2 += amp;

    }

    for (int i = FFT_N/2 - 10; i < FFT_N/2; i++) {

        fft_in_data[i].R1 = 100;
        fft_in_data[i].I1 = 50;
