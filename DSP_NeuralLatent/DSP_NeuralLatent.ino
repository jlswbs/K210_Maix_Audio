// Neural cellular automaton latent space generator //
    ...
    for (int iter = 0; iter < ITERATIONS; iter++) {
        for (int i = 0; i < BUFFER_SIZE; i++) {
            ...
            for (int h = 0; h < HIDDEN_SIZE; h++) {
                ...
                latent_sum += hidden_outputs[h];
            }

            float latent_state = latent_sum / (float)HIDDEN_SIZE;
            float raw_step = src[i] + latent_state * UPDATE;
            dst[i] = tanhf(raw_step * 1.5f);
        }
        float* temp = src;
        src = dst;
        dst = temp;
...
int audio_callback(void *ctx) {
    ...
    float compressed = tanhf(sample * 1.0f);
