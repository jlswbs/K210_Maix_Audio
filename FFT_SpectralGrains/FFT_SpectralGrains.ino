...
void loop() {
    ...
    for (int i = 0; i < FFT_N / 2; i++) {
        ...
        fft_in_data[i].R1 *= 0.85f;
        fft_in_data[i].I1 *= 0.85f;
        fft_in_data[i].R2 *= 0.85f;
        fft_in_data[i].I2 *= 0.85f;

    }

    if (rand() % 10 > 4) {

        int high_bin = (rand() % 150) + 100;
        int16_t amp = rand() % 12000;
        
        if (high_bin % 2 == 0) {
            fft_in_data[high_bin / 2].R1 += amp;
            fft_in_data[high_bin / 2].I1 += rand() % amp;
        } else {
            fft_in_data[high_bin / 2].R2 += amp;
            fft_in_data[high_bin / 2].I2 += rand() % amp;
        }
