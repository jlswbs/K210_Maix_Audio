// Spectral alienoid FFT hardware //
...
void loop() {
    ...
    static uint32_t flux = 0;
    static int16_t feedback_register = 0;
    flux++;

    feedback_register ^= audio_out[rand() % FFT_N];

    for (int i = 0; i < FFT_N / 2; i++) {

        if ((flux % 4) == 0) {
            fft_in_data[i].R1 ^= feedback_register; 
            fft_in_data[i].I1 &= (flux << 2);
        }

        if (i == (flux % (FFT_N / 2))) {
            fft_in_data[i].R1 += 20000; 
            fft_in_data[i].I2 -= 20000;
        }
    ...
    for (int i = 0; i < FFT_N / 2; i++) {
        ...
        int16_t out_r = fft_out_data[i].R1;
        int16_t noise_gate = (rand() % 100 > 95) ? 0 : out_r;
        audio_out[2 * i]     = noise_gate ^ (flux & 0xFF); 
        audio_out[2 * i + 1] = (fft_out_data[i].R2 >> 2) ^ feedback_register;

    }

    int drift = 5 + (flux % 12);
    if (flux % 200 == 0) drift = 150;
