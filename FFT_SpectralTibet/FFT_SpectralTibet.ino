...
void loop() {
    ...
    for (int i = 0; i < FFT_N / 2; i++) {
        ...
        if(i > 0 && i < (FFT_N / 2) - 1) {
            ...
            float diffuse = 0.03f;
            fft_in_data[i].R1 += (int16_t)(fft_in_data[i-1].R1 * diffuse - fft_in_data[i+1].R1 * diffuse);
            fft_in_data[i].I1 += (int16_t)(fft_in_data[i-1].I1 * diffuse - fft_in_data[i+1].I1 * diffuse);

        }

    }

    if (rand() % 300 == 0) fft_in_data[1].R1 += 6000;

    if (rand() % 200 == 0) fft_in_data[2].R1 += 8000;
