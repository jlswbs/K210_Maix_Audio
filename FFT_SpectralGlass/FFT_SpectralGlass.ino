...
void loop() {
    ...
    for (int i = 0; i < FFT_N / 2; i++) {
        ...
        float decay = (i % 16 == 0) ? 0.992f : 0.75f;
        
        fft_in_data[i].R1 *= decay;
        fft_in_data[i].I1 *= decay;
        fft_in_data[i].R2 *= decay;
        fft_in_data[i].I2 *= decay;

    }

    if (rand() % 10 > 6) {

        int high_bin = (rand() % 180) + 50; 
        int16_t amp = rand() % 8000;
        
        if (high_bin % 2 == 0) {
            fft_in_data[high_bin / 2].R1 += amp;
            fft_in_data[high_bin / 2].I1 += (rand() % 1000);
        } else {
            fft_in_data[high_bin / 2].R2 += amp;
            fft_in_data[high_bin / 2].I2 += (rand() % 1000);
        }
